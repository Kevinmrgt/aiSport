...
> Version déployée : `0.13.0-rc.8` au 2026-07-23, commit `f817073de7ed7220fbbc38d396f1d181811012bd`.
> Validation de la version : CI `29999207578` ; CD `29999526386`.
> Le SHA archivé et les empreintes de la remise `rc.8` figurent dans le
...
| Élément                            | Valeur                                                                                                                        |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Scénarios de recette comptabilisés | 65 ; CR-049 est un risque architectural suivi séparément et n'entre pas dans le dénominateur                                  |
| Scénarios avec résultat            | 65 : exécutions, observations ou contrôles automatisés avec une preuve associée                                               |
...
| SHA testé                          | baseline déployée `f817073de7ed7220fbbc38d396f1d181811012bd`                                                                  |
| Environnement                      | baseline `rc.8` en CI/production ; trois endpoints de santé en HTTP 200                                                       |
| Artefacts                          | preuves A20, A25 à A43 ; matrice user stories ; paquet `rc.8` généré, inspecté et vérifié                                     |

Les 65 scénarios ne sont pas tous des manipulations manuelles : le statut
...
### Contrôles de la version `0.13.0-rc.8` — 2026-07-23

| Contrôle                     | Résultat                                                                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------ |
| Audit, lint et types         | réussis localement et dans la CI `29999207578`                                                   |
| Tests                        | 261/261 : shared 14, API 179, Web 68, plus intégration PostgreSQL concurrente                    |
| Playwright accès jury        | 18/18 sur Chromium et Firefox, complétés par la recette navigateur de production                 |
| Builds et images Docker      | réussis dans les six jobs de CI                                                                  |
| CD                           | `29999526386` : migration, API, Web et smoke tests réussis                                       |
| Santé production             | liveness/readiness/Web HTTP 200 en `rc.8`, DB et IA `ok`                                         |
| Recette navigateur jury      | connexion, 29/30 partagé, front sans détails techniques/tarifaires et zéro erreur console        |
| Quota jury                   | 30 succès maximum ; persistance et concurrence vérifiées côté API/PostgreSQL                     |
...
| ----------------------------------- | ----------------------------- | --------------------------------- | -------------------------------------------------------- |
| Authentification et session         | CR-001 à CR-009 (9)           | 9 renseignés                      | B2-A25, B2-A30, accès jury rc.8, E2E OAuth `29833210488` |
...
| Coach, configuration et dashboard   | CR-036 à CR-041 (6)           | 6 renseignés                      | B2-A25, B2-A34, B2-A43                                   |
...
| Qualité, intégration et déploiement | CR-056 à CR-066 (11)          | 11 renseignés                     | B2-A19, B2-A22, B2-A38, B2-A42, B2-A43, manifeste        |
| **Total**                           | **65**                        | **65 résultats documentés**       | **index des annexes et présent cahier**                  |
...
| CR-005 | Connexion jury valide     | accès actif, saisir les identifiants temporaires                        | vraie session Auth.js dédiée puis redirection `/generate`        | tests unitaires + Playwright + recette production | Exécuté localement et en production sur `rc.8` ; session puis redirection `/generate`, B2-A42/B2-A43                     |
| CR-006 | Connexion jury invalide   | identifiant ou mot de passe erroné                                      | refus et message générique sans révéler la cause                 | tests unitaires + Playwright                      | Automatisé localement ; message accessible `role=alert`                                                                   |
| CR-007 | Révocation accès jury     | expirer, désactiver ou faire tourner le secret                          | session existante refusée et retour à `/login`                   | tests unitaires callback JWT                      | Automatisé localement ; expiration absolue et empreinte de configuration                                                  |
| CR-008 | Compteur jury partagé     | session jury, ouvrir `/generate`, générer une fois puis `/programs/generate` | compteur persistant commun aux deux formulaires, de 30 à 29  | API/PostgreSQL + recette navigateur production    | Exécuté sur `rc.8` : 29/30 restantes et même compteur affiché sur les deux pages, sans erreur console ; B2-A43              |
| CR-009 | Limite jury atteinte      | compte jury à 30 succès, demander une 31e génération                    | refus HTTP 429 `GENERATION_QUOTA_EXCEEDED`, sans appel IA ni création | tests API + intégration PostgreSQL concurrente | Automatisé sur `rc.8` : 30 réservations concurrentes acceptées, la suivante refusée ; compteur atomique et persistant      |
...
| ID     | Fonctionnalité                | Préconditions et actions                     | Résultat attendu                                                                | Preuve prévue            | Statut                                                                                                        |
| ------ | ----------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------- |
| CR-036 | Consulter l'espace Coach      | compte actif, ouvrir `/settings`             | accompagnement automatique expliqué en langage métier                           | test rendu/Web + E2E     | Exécuté sur `rc.8` : titre et étapes visibles dans le navigateur intégré ; B2-A43                            |
| CR-037 | Masquer les choix techniques  | ouvrir `/settings` et `/generate`            | aucun fournisseur, modèle, prix estimé, coût ou volume de sortie affiché         | tests composants + E2E   | Exécuté sur `rc.8` : non-exposition automatisée puis contre-recettée en production ; B2-A43                 |
...
| CR-058 | Qualité/build             | lint, typecheck, build sous Node 24                                                       | toutes commandes réussies                                                                                                                                  | logs CI + B2-A39/A41/A42/A43               | Exécuté - `rc.8` : lint, types, 261 tests et builds verts localement et dans la CI `29999207578`                                                                                                                                                                                                                              |
...
| CR-061 | CI complète               | pousser le SHA final                                                                      | tous les jobs obligatoires verts                                                                                                                           | run GitHub                                 | Exécuté - Run final `29999207578` réussi sur `f817073`                                                                                                                                                                                                                                                                        |
...
| CR-066 | Comptabilisation du quota | session jury puis succès, erreur IA/DB et suppression ; comparer une session Google       | seuls les succès jury consomment ; erreurs IA/DB remboursées ; suppression sans remboursement ; Google illimité                                           | tests services/API + PostgreSQL concurrent | Automatisé sur `rc.8` : règles de réservation/libération et distinction jury/Google couvertes ; B2-A42/B2-A43                                                                                                                                                                                                                 |

## Critère de clôture C2.3.1

Les 65 scénarios de recette sont reliés à une preuve ou à une réserve explicite.
CR-063 est validé par la construction, l'inspection, la
décompression et la comparaison des empreintes du paquet `rc.8`.
...
CR-008, CR-009 et CR-066 couvrent le quota jury partagé, son plafond atomique
et ses règles de comptabilisation. CR-049 demeure un risque architectural hors dénominateur. Les tests
