...
| Scénarios réussis                   | Non agrégés ; 212 tests API/Web/shared et des parcours authentifiés réels sont détaillés sans conversion en pourcentage fonctionnel |
...
| CR-052 | Pages authentifiées     | axe sur generate, programmes, listes, détail, dashboard, settings | aucune violation applicable non traitée                   | Playwright avec vrai storage state | Exécution partielle : `/generate` authentifié testé sans violation critique/sérieuse ; autres pages et audit humain restent à contrôler                |
...
| CR-054 | Reflow/mobile           | 320 px CSS et viewport mobile                                     | aucune perte d'information/action ni scroll 2D injustifié | captures + audit manuel            | Exécution partielle réelle : pages publiques à 320 px et `/generate` authentifié à 390 × 844 sans débordement horizontal ; reflow 400 % humain restant |
