...
> Version déployée : `0.13.0-rc.7` au 2026-07-23, commit `d42e7f2c8fc86f26c46f850d32eb748870c6140d`.
> Validation de la version : CI `29994929981` ; CD `29995297354`.
> Le SHA archivé et les empreintes de la remise `rc.7` figurent dans le
...
| SHA testé                          | baseline déployée `d42e7f2c8fc86f26c46f850d32eb748870c6140d`                                                                  |
| Environnement                      | baseline `rc.7` en CI/production ; trois endpoints de santé en HTTP 200                                                       |
| Artefacts                          | preuves A20, A25 à A42 ; matrice user stories ; paquet `rc.7` généré, inspecté et vérifié                                     |
...
### Contrôles de la version `0.13.0-rc.7` — 2026-07-23

| Contrôle                     | Résultat                                                                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------ |
| Audit, lint et types         | réussis localement et dans la CI `29994929981`                                                   |
| Tests                        | 267/267 : shared 14, API 179, Web 74, plus intégration PostgreSQL concurrente                    |
...
| Santé production             | liveness/readiness/Web HTTP 200 en `rc.7`, DB et IA `ok`                                         |
| Recette navigateur jury      | connexion valide, 29/30 restantes après une génération, compteur partagé et zéro erreur console |
...
| ----------------------------------- | ----------------------------- | --------------------------------- | -------------------------------------------------------- |
| Authentification et session         | CR-001 à CR-009 (9)           | 9 renseignés                      | B2-A25, B2-A30, accès jury rc.7, E2E OAuth `29833210488` |
...
| CR-005 | Connexion jury valide     | accès actif, saisir les identifiants temporaires                        | vraie session Auth.js dédiée puis redirection `/generate`        | tests unitaires + Playwright + recette production | Exécuté localement et en production sur `rc.7` ; session puis redirection `/generate`, B2-A42                             |
...
| CR-058 | Qualité/build             | lint, typecheck, build sous Node 24                                                       | toutes commandes réussies                                                                                                                                  | logs CI + B2-A39/A41/A42                   | Exécuté - `rc.7` : lint, types, 267 tests et builds verts localement et dans la CI `29994929981`                                                                                                                                                                                                                              |
...
| CR-061 | CI complète               | pousser le SHA final                                                                      | tous les jobs obligatoires verts                                                                                                                           | run GitHub                                 | Exécuté - Run final `29994929981` réussi sur `d42e7f2`                                                                                                                                                                                                                                                                        |
...
décompression et la comparaison des empreintes du paquet `rc.7`.
