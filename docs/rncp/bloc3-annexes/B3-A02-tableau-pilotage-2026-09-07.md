...
> Date de mesure : **2026-09-07, 09:56–10:56 CEST (Europe/Paris)**  
> Baseline contrôlée : **`0.13.0-rc.8`**  
> SHA contrôlé : **`d950b6b790a8b11153995bf817b7cb0d583d36da`**  
> Branche de travail lors du contrôle : `codex/bloc3-finalisation`  
> Référence distante : `origin/main` au même SHA  
> Correctif de dépendances : commit local **`7fc5f01`**, construit sur cette
> baseline ; CI distante encore attendue  
...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | :----: | ------------------------------------------------------------------------------------------------------ |
| Avancement             | Baseline `0.13.0-rc.8` en production ; correctif de dépendances commité en `7fc5f01`, sans CI distante ni déploiement                   | Orange | Conserver `rc.8` pour la démo et faire valider le lockfile corrigé par une CI avant gel                |
| Délais                 | CI et CD du SHA courant réussies le 2026-08-20 ; monitoring réussi le 2026-09-07 ; date contractuelle de soutenance absente des sources | Orange | Piloter les actions restantes par échéances internes et renseigner la date jury dès qu'elle est connue |
...
| Sécurité               | Overrides qualifiés vers `browserslist@4.28.7` et `postcss-selector-parser@6.1.3` ; audits complet et production à 0 vulnérabilité      |  Vert  | Soumettre le lockfile corrigé à la CI puis remplacer les overrides quand les parents intègrent le fix  |
| Production             | 150/150 réponses valides ; p95 maximum 246,42 ms ; Web/API/DB/configuration IA prêts en `rc.8`                                          |  Vert  | Maintenir le monitoring et refaire la mesure le jour de la répétition                                  |
...
| Validation utilisateur | Grille créée, aucun score réel collecté                                                                                                 | Orange | Faire exécuter et signer une validation avant la soutenance                                            |
...
- **9/9 verts localement**, soit **100 %**, après audits à 0 et smoke 54/54
  avec `workers=1` ;
- le correctif de lockfile est **commité en `7fc5f01` mais non validé par une
  nouvelle CI** : la traçabilité de livraison demeure orange.

**Décision : `GO technique local sous réserves`, et `NO-GO` pour un gel final
tant que le commit `7fc5f01` n'a pas une CI verte.**
...
| AV-02  | Gates vertes / gates applicables               | 9/9 ; rouge si une gate sécurité échoue    | 9/9 localement ; commit `7fc5f01`, CI distante encore attendue        |    ↑     | Orange | Kevin — pilotage/QA  | À chaque gel                    | Faire valider le lockfile corrigé par la CI                                                      |
| AV-03  | SHA production conforme à `origin/main`        | Même SHA                                   | CI et CD sur `d950b6b…`, identique à `origin/main`                    |    →     |  Vert  | Kevin — DevOps       | À chaque déploiement            | Runs CI `32393765258` et CD `32394435200`                                                        |
...
| DEL-01 | Délai de livraison technique                   | CI puis CD réussies sur le SHA livré       | CI : 20 août 16:45–16:52 UTC ; CD : 16:52–16:55 UTC                   |    →     |  Vert  | Kevin — DevOps       | À chaque livraison              | GitHub Actions, mêmes SHA et conclusion `success`                                                |
| DEL-02 | Fraîcheur du contrôle de production            | Dernier contrôle < 24 h                    | Monitoring réussi le 7 septembre 04:43 UTC ; contrôle local 08:01 UTC |    →     |  Vert  | Kevin — exploitation | Horaire automatisé + avant démo | Run monitoring `34084157427` et campagne locale 150 requêtes                                     |
...
| SEC-01  | Vulnérabilités connues `pnpm audit --audit-level=low`  | 0 ; rouge dès 1 high/critical                           | 0 audit complet ; 0 audit production après overrides qualifiés       |    ↑     |  Vert  | Kevin — sécurité        | Quotidien avant gel + CI    | Valider le lockfile par CI ; surveiller puis retirer les overrides devenus inutiles |
| PROD-01 | Disponibilité ponctuelle = réponses valides / requêtes | 100 % ; alerte dès un échec                             | 150/150, soit 100 %, sur Web/liveness/readiness                      |    →     |  Vert  | Kevin — exploitation    | Horaire + avant démo        | `pnpm measure:production-health -- 50`                                              |
| PROD-02 | Latence p95 des healthchecks                           | <= 1 000 ms par endpoint                                | Web 207,28 ms ; liveness 246,42 ms ; readiness 182,37 ms             |    ↑     |  Vert  | Kevin — exploitation    | Hebdomadaire + avant démo   | 50 requêtes séquentielles par endpoint ; ce n'est pas un test de charge             |
| PROD-03 | Cohérence version/santé                                | 3/3 HTTP 200 et version attendue ; readiness DB/IA `ok` | 3/3 HTTP 200 en `0.13.0-rc.8`, DB `ok`, IA `ok`                      |    →     |  Vert  | Kevin — release         | Après déploiement           | Réponses JSON lues à 08:01 UTC                                                      |
...
| RH-04   | WIP critique simultané                                  | <= 1 sujet critique                  | Non mesuré formellement                                            |    ?     |      Orange       | Kevin — pilotage             | Quotidienne                      | Limiter à une action rouge active : CI du correctif, puis coûts/charge et continuité |
...
| Validation CI du SHA courant | Après intégration sur `main` | 2026-08-20, six jobs réussis                     | Oui, séquence respectée | Run `32393765258`                       | Baseline techniquement validée à cette date               |
| Déploiement du SHA courant   | Après CI verte               | 2026-08-20, migration puis API puis Web réussies |   Oui, ordre respecté   | Run `32394435200`                       | Production alignée sur la baseline                        |
...
| --------- | ---------------------------------------------------------------- | --: | --: | ----: | :------: | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | :----: |
| R-SEC-01  | Avis transitifs détectés puis remédiés localement par overrides  |   1 |   4 |     4 |    ↑     | Audits complet/prod à 0 ; versions corrigées verrouillées      | Kevin sécurité — obtenir une CI verte sur le futur SHA puis surveiller les versions parentes       |  Vert  |
...
| R-DEMO-01 | Démo bloquée par réseau, OAuth ou latence                        |   3 |   5 |    15 |    ↑     | Production saine, accès jury, quota, seed, mode local          | Kevin démo — répétition complète, compte vérifié et captures de secours à J-2                      | Orange |
...
| R-VAL-01  | Validation et satisfaction client non collectées                 |   4 |   4 |    16 |    →     | Grille SAT-01 à SAT-07 prête                                   | Kevin produit — faire tester cinq tâches et faire signer la décision avant la soutenance           | Orange |
...
| :------------------: | -------------------------------------- | ----------------------------------------------------------------------------------------- | -------------------------- | ------------------------------------- | ----------------------------------------------------------- |
| P0 — fait localement | Résoudre l'audit de dépendances        | Audits complet et production à 0 vulnérabilité                                            | Kevin — sécurité           | Réalisé le 2026-09-07 ; CI restante   | Overrides, lockfile, journal ci-dessous                     |
...
|     P0 — restant     | Publier et valider le correctif        | CI complète verte sur `7fc5f01` et audits à 0                                             | Kevin — DevOps/sécurité    | Avant gel                             | Futur run CI du commit corrigé                              |
...
|          P1          | Valider la démonstration avec un tiers | SAT-01 à SAT-07 renseignés, réserves transformées en actions                              | Kevin — produit            | J-2                                   | `B3-A06-comptes-rendus-validation-satisfaction.md` complété |
...
Ordre décidé : **CI du correctif → coûts/charge → continuité → validation
tierce**. Les deux remédiations locales ne sont pas confondues avec une
livraison validée tant que le commit `7fc5f01` n'a pas une CI distante verte.
...
- CI du SHA courant :
  `https://github.com/Kevinmrgt/aiSport/actions/runs/32393765258` ;
- CD du SHA courant :
  `https://github.com/Kevinmrgt/aiSport/actions/runs/32394435200` ;
...
- Les audits complet et production sont à 0 après correction locale. Le
  lockfile modifié est rattaché au commit `7fc5f01`, mais pas à une CI ni à un
  déploiement ; les runs du 2026-08-20 restent les preuves du SHA de base.
- La satisfaction et la validation d'un commanditaire restent non mesurées.
...
disponible et performante sur ses healthchecks. Les actions locales sécurité et
E2E sont maintenant closes : audits à 0 et smoke sérialisé 54/54. La décision
reste néanmoins **GO technique local sous réserves** jusqu'à validation du
commit `7fc5f01` par la CI ; les coûts, la charge, la continuité et la
validation utilisateur doivent toujours être objectivés.
