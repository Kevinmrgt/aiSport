...
| Pull request publication `rc.5`    | `https://github.com/Kevinmrgt/aiSport/pull/49` |
...
| CI baseline `main`                 | run `29916228789` — 6 jobs réussis             |
| CD baseline `main`                 | run `29916573448` — migration, API, Web et smoke réussis |
...
remise peut être postérieur à `b63280f` lorsqu'il ne contient que des
corrections documentaires ; la baseline applicative déployée reste alors
explicitement `b63280f36e44b02d5654a7f4e2caa8413e446bcb`.
...
La version courante `0.13.0-rc.5` corrige l'alerte vocale de redirection et la
région de statut des paramètres. La PR `#49`, la CI `29916228789`, la CD
`29916573448`, les trois healthchecks HTTP 200 et la validation NVDA déclarée
par l'utilisateur sont consignés dans B2-A41.
...
| Qualité et performance                           | dossier §3, B2-A29/B2-A41, CI `29916228789`                   |
...
- [x] publication de `0.13.0-rc.5` : PR `#49`, CI `29916228789`, CD
      `29916573448`, 241 tests et trois healthchecks HTTP 200 en version `rc.5` ;
- [x] contre-recette NVDA `rc.5` déclarée validée par l'utilisateur, sans
      ajouter de navigateur, version, verbatim ou protocole non fourni ;
...
- [x] baseline `main` `b63280f` déployée avec migration et smoke tests par le run `29916573448` ;
