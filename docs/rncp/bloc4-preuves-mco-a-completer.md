...
- Aucun run du workflow de monitoring, artefact, issue GitHub ou capture n'est présenté comme déjà obtenu dans cette checklist.
...
| --------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| P0        | Run vert `Monitoring - Production health`       | Lancer le workflow manuellement ou attendre son passage horaire ; capturer le nom, la date, les deux étapes de contrôle et le statut succès.     | Montre l'exécution effective de la supervision GitHub.                                                   |
| P0        | Artefact `production-health-report` du même run | Ouvrir ou télécharger l'artefact ; conserver le rapport qui contient les URLs, statuts et payloads JSON.                                         | Prouve les sondes et leurs réponses, pas seulement le YAML.                                              |
...
| P2        | Preuve d'issue automatique de monitoring        | Conserver l'issue si un incident réel la génère. Ne pas provoquer une panne de production seulement pour obtenir cette capture.                  | Confirme le signalement automatique, si l'événement existe.                                              |
